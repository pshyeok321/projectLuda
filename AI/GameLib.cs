...
public static class GameLib
{
    ...
    {
        return Physics.BoxCastAll(transform.position, transform.lossyScale * 10,
    ...
    public static bool DetectObject(Camera sight, BoxCollider bc)
    {
        Plane[] ps = GeometryUtility.CalculateFrustumPlanes(sight);
        
        return GeometryUtility.TestPlanesAABB(ps, bc.bounds);
    }    
    
    public static bool DetectCharacter2(Camera sight, CapsuleCollider cc)
    {
        Plane[] ps = GeometryUtility.CalculateFrustumPlanes(sight);
    ...
    public static bool DetectCharacter3(Camera sight, CapsuleCollider cc, bool isrunning)
    {
        if (isrunning)
        {
            Plane[] ps = GeometryUtility.CalculateFrustumPlanes(sight);
